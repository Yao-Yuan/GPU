...
#if CRYPTO_BYTES<=32
__constant__ static const u32 columnconstant[2] = { 0x30201000, 0x70605040 };
__constant__ static const u8 shiftvalues[2][8] = { {0, 1, 2, 3, 4, 5, 6, 7}, {1, 3, 5, 7, 0, 2, 4, 6} };
#else
__constant__ static const u32 columnconstant[4] = { 0x30201000, 0x70605040, 0xb0a09080, 0xf0e0d0c0 };
__constant__ static const u8 shiftvalues[2][8] = { {0, 1, 2, 3, 4, 5, 6, 11}, {1, 3, 5, 11, 0, 2, 4, 6} };
#endif
...
*/
		if(threadIdx.x<COLWORDS){
			x[STATEWORDS-COLWORDS+threadIdx.x] ^= columnconstant[threadIdx.x]^constant;			
		}
		__syncthreads();
    }
...
//	#pragma unroll
	for(i = 0 ;i<1025&&result_flag == 0; i++)
...
//  while(s.last_padding_block == 0 && result_flag == 0)
  ...
  /* return truncated hash value */
    output_hash[tID]=((u8*)ctx)[BYTESLICE(tID+CRYPTO_BYTES)];
	output_hash[tID+32]=((u8*)ctx)[BYTESLICE(tID+CRYPTO_BYTES + 32)];
