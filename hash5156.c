  ...
  for(constant=0; constant<(0x01010101*ROUNDS); constant+=0x01010101)
  {
    ...
    if (q==0)
    {
     /* for (j=0; j<COLWORDS; j++)
		...
		if(threadIdx.x<4){
			x[threadIdx.x] ^=columnconstant[threadIdx.x]^constant;			
		}
		__syncthreads();
    }
    else
...
*/
		if(threadIdx.x<4){
			x[28+threadIdx.x] ^= columnconstant[threadIdx.x]^constant;			
		}
		__syncthreads();
    }
   /* for (i=0; i<8; i++)
    {
	  ...
	  if(threadIdx.x<4){
	     tmp[threadIdx.x]=x[i*4+threadIdx.x];
	  }
	  __syncthreads();
	...
	if(threadIdx.x<8){
	  for (j=0; j<COLWORDS; j++)
        tmp[j] = x[threadIdx.x*COLWORDS+j];
      for (j=0; j<STATECOLS; j++)
        ((u8*)x)[threadIdx.x*STATECOLS+j] = S[((u8*)tmp)[(j+shiftvalues[q][threadIdx.x])%STATECOLS]];
	}
	__syncthreads();	
   ...
   /* for (j=0; j<COLWORDS; j++)
	...
	if(threadIdx.x<4){
	  mixbytes((u32(*)[4])x, tmp,threadIdx.x);
	}
	__syncthreads();
  }
}
...
/*	if(threadIdx.x!=0&&threadIdx.x<s.bytes_in_block){
		buffer[BYTESLICE(threadIdx.x)]=in[threadIdx.x];
	}
	__syncthreads();*/
  }else {
	 for (i = 0; i < s.bytes_in_block; i++)
    buffer[BYTESLICE(i)] = in[i];
	/*if(threadIdx.x<s.bytes_in_block){
		buffer[BYTESLICE(threadIdx.x)] = in[threadIdx.x];
	}
	__syncthreads();*/
  }
...
/*  if(blockIdx.x==0){
  ...
  */
  ...
  __shared__ char temphash[65];
 // int num;
  out[0]='\0';
  //__shared__ char input[INPUT_SIZE+1+NONCE_SIZE];
  //int num;
  //input[0]=nonce[tid];
  //for(num=1;num<
 // temp=nonce[tid];
  //unsigned char outtemp[65];
...
//	printf("%s\n",in);
 ...
 /* for (i = STATEBYTES-CRYPTO_BYTES; i < STATEBYTES; i++)
    temphash[i-(STATEBYTES-CRYPTO_BYTES)] = ((u8*)ctx)[BYTESLICE(i)];
*/
	temphash[tid]=((u8*)ctx)[BYTESLICE(tid+64)];
	temphash[tid+32]=((u8*)ctx)[BYTESLICE(tid+96)];

 /*  if(blockIdx.x==0){
     for(num=0;num<64;num++){
		printf("%02X",temphash[num]);
	 }	 
   }*/
 ...
 // printf("%c",nonce[blockIdx.x]);
   if(check_hash(temphash))
